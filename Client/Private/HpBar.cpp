...
void CHpBar::Update(_float fTimeDelta)
{
    ...
    else
    {
        ...
        else if (ownerType == 1) // 좀비
        {
            CZombie* pSkeleton = dynamic_cast<CZombie*>(m_pOwner);
            if (*pSkeleton->Get_TakeHit())
            {
                m_fSizeX = m_fOriginalSizeX * m_pOwner->Get_HpRatio();
                pSkeleton->Set_TakeHit(false);
        ...
        else if (ownerType == 2) // 크리퍼
        {
            CCreeper* pSkeleton = dynamic_cast<CCreeper*>(m_pOwner);
            if (*pSkeleton->Get_TakeHit())
            {
                m_fSizeX = m_fOriginalSizeX * m_pOwner->Get_HpRatio();
                pSkeleton->Set_TakeHit(false);
        ...
        {
            CSkeletonVanguard* pSkeleton = dynamic_cast<CSkeletonVanguard*>(m_pOwner);
            if (*pSkeleton->Get_TakeHit())
            {
                m_fSizeX = m_fOriginalSizeX * m_pOwner->Get_HpRatio();
                pSkeleton->Set_TakeHit(false);
