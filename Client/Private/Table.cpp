...
CTable::CTable(const CTable& Prototype)
...
void CTable::Priority_Update(_float fTimeDelta)
{

}

void CTable::Update(_float fTimeDelta)
{

}

void CTable::Late_Update(_float fTimeDelta)
{
...
HRESULT CTable::Ready_Components()
{
	if (FAILED(__super::Add_Component(ETOI(LEVEL::DUNGEON), TEXT("Prototype_Component_Shader_VtxMesh"),
		TEXT("Com_Shader"), reinterpret_cast<CComponent**>(&m_pShaderCom))))
		return E_FAIL;

	if (FAILED(__super::Add_Component(ETOI(LEVEL::DUNGEON), TEXT("Prototype_Component_Model_MapTable"),
...
CTable* CTable::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	{
		MSG_BOX("Failed to Created : CTable");
...
CGameObject* CTable::Clone(void* pArg)
{
	...
	{
		MSG_BOX("Failed to Cloned : CTable");
		Safe_Release(pInstance);
	}
	return pInstance;
}
